...
module.exports = postcss.plugin("postcss-pxtorem-multi", (options = []) => {
  const optss = {};
  options.forEach(opts => {
    optss[opts.type] = Object.assign({}, defaults, opts);
  });

  return css => {
    const type = getTypeDeclaration(css.nodes);
    const opts = optss[type];
    if(!opts) {
      return;
    }
...
function getTypeDeclaration(nodes) {
  let type = ''
  const typeReg = /root value ([a-zA-z]+)/;
  for(var i=0;i< nodes.length; i+=1) {
    const node = nodes[i];
    if(node.type === 'comment') {
      const regResult = typeReg.exec(node.text);
      if(regResult) {
        type = regResult[1];
      }
    }
